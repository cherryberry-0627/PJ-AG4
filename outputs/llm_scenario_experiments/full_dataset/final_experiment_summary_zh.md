...
- 最新对齐汇总文件：`aligned_llm_modes_seed7_r12_context_engineering_summary.csv`
...
| `llm-context-engineering` | 直接 LLM action，但 prompt 加入压缩历史上下文和 signals |
| `llm-adaptive` | LLM 不直接出 action，只更新 bounded strategy state |
| `llm-context-adaptive-engineering` | 在 adaptive 基础上加入 context engineering payload |
...
| --- | --- | ---: | ---: | ---: | ---: | --- |
| `baseline` | `llm-context-adaptive-engineering` | 3823.67 | 99.20% | 3 | 0.58 | 稳态场景下 context-adaptive 收益最高，但 defaults 略多 |
| `price_war` | `llm-adaptive` | 3597.09 | 99.78% | 0 | 0.00 | 价格战中原 adaptive 更稳，context-adaptive 略低 |
| `supply_shock` | `llm-context-adaptive-engineering` | 4326.60 | 96.08% | 7 | 3.65 | 最强展示场景，context-adaptive 同时提高 profit 并大幅降低 backlog |
| `high_volatility` | `llm-context-adaptive-engineering` | 3745.00 | 99.27% | 2 | 1.10 | 波动场景下 context signals 帮助策略更稳 |
| `no_reputation` | `llm-context-adaptive-engineering` | 3693.09 | 98.96% | 5 | 2.63 | 声誉关闭后，历史上下文补充了部分趋势信息 |
| `no_transfer` | `llm-context-adaptive-engineering` | 3591.06 | 98.29% | 7 | 0.00 | 无 transfer 兜底时，context-adaptive 仍能压住 backlog |

## 关键发现

### Finding 1：最终推荐模式是 `llm-context-adaptive-engineering`

在 6 个场景中，`llm-context-adaptive-engineering` 有 5 个场景 profit 第一：
...
唯一例外是 `price_war`：`llm-adaptive` profit 为 3597.09，`llm-context-adaptive-engineering` 为 3501.24。

结论：最终展示时可以把 `llm-context-adaptive-engineering` 作为主模型，把 `price_war` 作为局限讨论。

### Finding 2：Context engineering 单独加到 direct LLM 上并不稳定

`llm-context-engineering` 不是稳定提升：
...
答辩表达：context engineering 的价值不是简单地把历史塞进 prompt，而是要和 bounded adaptive strategy update 结合。
...
| `llm-context-engineering` | 3696.31 | 95.10% | 9 | 11.19 | 50.31 |
| `llm-adaptive` | 4237.78 | 95.30% | 7 | 10.92 | 92.52 |
| `llm-context-adaptive-engineering` | 4326.60 | 96.08% | 7 | 3.65 | 83.96 |

相比旧 `llm-context-adaptive`，新版 context engineering 在 `supply_shock` 中：
...
- `llm-context-adaptive-engineering` 进一步让策略更新有历史证据和 signal 标签。

## 推荐答辩叙事

1. 先讲市场机制：客户分层、二阶段 allocation、transfer、SLA backlog。
2. 再讲 agent 设计：从 heuristic 到 direct LLM，再到 bounded adaptive。
3. 重点解释 context engineering：不是长 prompt，而是选择、压缩和标注历史证据。
...
`llm-context-adaptive-engineering` 是当前项目最适合作为最终展示的 agent 模式：它把市场机制、agent 对抗、LLM 自适应和 context engineering 结合起来，并且在 6 个场景中的 5 个取得最高 profit，尤其在 `supply_shock` 场景中显著降低 backlog。
